...
export const projects: Project[] = [
  {
    ...
    highlights: [
      ...
      '회원가입·로그인 화면, 인증 상태 기반 사용자 흐름, 사용자 상태 관리, 딥링크 유입 이후 앱 진입 흐름 구현',
  ...
  },
  {
    ...
    summary:
      '기존 Pertineo 서비스의 사용자 입력·분석 요청 흐름을 바탕으로, AI Rookie 버전에서 FastAPI 기반 API 구조와 제품 서버의 책임 경계를 설계하고 있습니다.',
    ...
    highlights: [
      ...
      '기존 서비스 경험을 기반으로 FastAPI 기반 API 구조 설계',
      'AI 분석 서비스와 제품 서버/API의 책임 경계 분리',
    ],
    metrics: ['기존 Pertineo client public repo 운영', 'AI Rookie 버전 현재 개발 중', '기존 서비스의 frontend 기능 구현 경험과 FastAPI 기반 API 설계 경험 연결'],
    challenge:
      'AI 분석 요청은 사용자 입력, 분석 상태, 결과 조회, 오류 처리 흐름이 길게 이어지는 기능입니다. AI Rookie 버전에서는 기존 Spring 코드를 단순히 문법 변환하는 것이 아니라, 기존 Pertineo의 기능과 요구사항을 바탕으로 사용자 경험과 제품 API가 안정적으로 연결되는 구조를 설계해야 했습니다.',
    solution: [
      '기존 프론트엔드에서 사용자 입력, 분석 대기, SSE streaming 처리 등 실제 사용자 플로우를 구현하며 도메인을 파악했습니다.',
      'AI Rookie 버전에서는 AI 분석 서비스와 제품 서버/API의 책임 경계를 분리했습니다.',
      '분석 요청, 응답, 예외 처리, 로깅을 추적 가능한 FastAPI 기반 API 구조로 설계하고 있습니다.',
    ],
